export const expresionsRegulars = {
    ...
    maxtextonumberRegex : /^[a-zA-ZñÑ0-9\s.,-]{5,200}$/, // solo se pueden ingresar letras a partir de 3 caracteres
...
export const MaxText_number_Expression = (value,setHook) => {
    let valor = value
      if (!expresionsRegulars.maxtextonumberRegex.test(valor) || valor.length < 5 ) {
        if(valor.length > 200 || valor.length < 5) {
          return(
              setHook({
                  error : true,
                  message : 'Minimo 5 carácteres, Máximo 200 carácteres. ',
                })
                // setDiseabledSubmitSession(false)
          )
        }
        return(
            setHook({
                error : true,
                message : 'Ingrese los datos solicitados sin caracteres especiales y tener minimo 5 caracteres.',
              })
              // setDiseabledNextEmpresa(true)
        )
        
      }
      else if(valor.length > 200 || valor.length < 5) {
        return(
            setHook({
                error : true,
                message : 'Minimo 5 carácteres, Máximo 200 carácteres. ',
